...
#importing datasets  
data_set= pd.read_csv("C:/Users/User/Desktop/SKIH3013 PROJECT/reddit_vm.csv")  
...
# In this, we have set 1 as the score for neutral sentiment.
# Once the score is above 1, sentiment is Positive i.e. People support COVID Vaccination
# If the score is below 1, sentiment is Negative i.e. People are against COVID Vaccination (Anti Vaxxers)
...
def predict():
    ...
    tokenized_text = lemmatized_text.split()
    ...
    # Vectorize the processed text using the same TfidfVectorizer used for training
    user_input_vectorized = vectorizer.transform([processed_text])

    # Predict sentiment using the trained model
    sentiment_prediction = SVCmodel.predict(user_input_vectorized)

    # Convert sentiment label to string
    sentiment_str = ''
    if sentiment_prediction == 0:
        sentiment_str = "Negative Sentiment"
    elif sentiment_prediction == 1:
        sentiment_str = "Neutral Sentiment"
    elif sentiment_prediction == 2:
        sentiment_str = "Positive Sentiment"

    return render_template('index.html', user_input=user_input, sentiment=sentiment_str)
